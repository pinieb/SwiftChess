...
//        for i in 1 ... maxDepth {
            scoredMoves = minimax(moves2: moves, depth: maxDepth)
//        }

        completion(selectMove())
    }

    private func minimax(moves2: [Move], depth: Int) -> [(move: Move, score: Double)] {
        var scoredMoves = [(move: Move, score: Double)]()
        ...
        for move in moves2 {
            ...
            if depth > 1 {
                ...
                if !newMoves.isEmpty {
                    score = minimax(moves2: newMoves, depth: depth - 1)[0].score
                }
            }

            if score == nil {
                score = evaluator.evaluate(position: position)
            }
